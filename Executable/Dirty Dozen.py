...
class Piece:
	...
	def movable(self, direction):
		"""determines if the piece is movable either directly or by pushing in a given direction. 
		'direction' should be a tuple or list of a relative coordinate.
		eg. (0, -1) for up and (1, 0) for right"""
		clear = self.clear(direction)
		if type(clear) == list:
			for piece in clear:
				if not piece.movable(direction):
					return False
			return True
		else:
			return clear
	...
	def determine_axis(self):
		"""Attempts to determine whether the piece can be moved on the x axis andor the y axis."""
		xaxis = ((-1, 0), (1, 0))
		yaxis = ((0, -1), (0, 1))
		...
		for tile_pos in self.tiles:
			if self.board.get_piece(self.adjust(tile_pos, xaxis[0])) not in [None, self]:
				leftFree = False
			if self.board.get_piece(self.adjust(tile_pos, xaxis[1])) not in [None, self]:
				rightFree = False
			if self.board.get_piece(self.adjust(tile_pos, yaxis[0])) not in [None, self]:
				upFree = False
			if self.board.get_piece(self.adjust(tile_pos, yaxis[1])) not in [None, self]:
				downFree = False
		self.xfree = leftFree or rightFree
		self.yfree = upFree or downFree
		if self.xfree ^ self.yfree: #'^' means XOR
	...
	def determine_boundaries(self):
		...
		#Sets the margins as the default boundaries
		self.first_boundary = marginLength
		self.second_boundary = marginLength + (boardLength - 1) * tileLength

		#Defines the first boundary
		counter = self.pos[index]
		while counter >= 0:
			exact_coord = marginLength + tileLength * counter
			for rel_pos in Piece.tileKey[self.shape]:
				search_tile = self.adjust(self.pos, rel_pos)
				search_tile[index] = counter + rel_pos[index] - 1
				if self.board.get_piece(search_tile) not in [None, self] and self.first_boundary < exact_coord:
					self.first_boundary = exact_coord
			counter -= 1

		#Defines the second boundary
		counter = self.pos[index]
		while counter < boardLength:
			...
			for rel_pos in Piece.tileKey[self.shape]:
				search_tile = self.adjust(self.pos, rel_pos)
				search_tile[index] = counter + rel_pos[index] + 1
				if self.board.get_piece(search_tile) not in [None, self] and self.second_boundary > exact_coord:
